...
**3. 자동화 투자 대비 효과 (ROI)**
- **손익분기점 계산:** `구현 시간 ÷ (1회 수동 실행 시간 × 주간 실행 횟수)`
- 예: 구현 4시간, 수동 실행 20분, 주 2회 반복 → `240분 ÷ (20분 × 2회)` = **6주 후 손익분기**
- 손익분기가 빠를수록 자동화 우선 순위가 높습니다

---

### 테스트 피라미드 설계

UI / API / E2E 3개 레이어로 설계합니다.
...
| 항목 | 판단 이유 |
|------|-----------|
| 주문 상태 조회 (정상/에러/없는 주문) | 핵심 비즈니스 로직, 실행 빠름 |
...
| 접근성(a11y) 최종 검증 | 자동화 툴로 보조하되 맥락적 판단은 수동으로 수행 |
